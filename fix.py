...
class FixMap(mapreduce.Map):
    def map(self, k, hamming_str):
        position = 0
        fixed_code = list(hamming_str)
        ...
        if t8 != hamming_str[7]:
            position += 8
        if position != 0:
            fixed_code[position-1] = '0' if hamming_str[position-1] == '1' else '1'
        self.emit(k, "".join(fixed_code))


class FixReduce(mapreduce.Reduce):
